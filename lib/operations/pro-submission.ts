...
export async function handleProSubmission(
  ...
  try {
    ...
    // 2. Find instrument in protocol and get safety monitoring rules
    const protocol = study.protocol as {
      instruments?: Instrument[] | Record<string, Instrument>
      safetyMonitoring?: { proAlerts?: SafetyProAlert[] }
    } | null
    ...
    // Get safety alerts from protocol (generated by Safety Agent)
    const proAlerts: SafetyProAlert[] = protocol?.safetyMonitoring?.proAlerts || []
    console.log(`[PRO] Safety alerts available: ${proAlerts.length}`)
    ...
    // 6. Evaluate safety and create alerts using protocol rules
