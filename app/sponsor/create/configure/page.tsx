    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-stone-500">Loading study options...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="mb-8 animate-fade-in">
        ...
        <h1 className="font-display text-2xl sm:text-3xl text-stone-900">{intervention}</h1>
        {discoveryData?.summary && (
          <p className="mt-2 text-stone-500">{discoveryData.summary}</p>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 stagger-children">
        ...
        <Card variant="default" padding="md">
          <h2 className="font-semibold text-stone-900 mb-1">Population</h2>
          <p className="text-sm text-stone-500 mb-4">Who are you studying?</p>
            ...
            {populations.map((option, index) => (
              ...
              >
                <input
                  ...
                  className="w-4 h-4 mt-0.5 text-teal-600 border-stone-300 focus:ring-teal-500"
                />
                ...
                {index === 0 && discoveryData && (
                  <span className="text-amber-600 text-xs font-medium bg-amber-50 px-2 py-1 rounded-full whitespace-nowrap">Recommended</span>
                )}
              </label>
            ))}
          </div>
        ...
        {/* Treatment Stage */}
        <Card variant="default" padding="md">
          <h2 className="font-semibold text-stone-900 mb-1">Treatment Stage</h2>
          <p className="text-sm text-stone-500 mb-4">What phase of treatment?</p>
          <div className="space-y-3">
        ...
        {/* Primary Endpoint */}
        <Card variant="default" padding="md">
          <h2 className="font-semibold text-stone-900 mb-1">Primary Endpoint</h2>
          <p className="text-sm text-stone-500 mb-4">What&apos;s the main outcome you&apos;re measuring?</p>
          <div className="space-y-3">
            {endpoints.map((option) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  {option.rationale && (
                    <p className="text-sm text-stone-500 mt-0.5">{option.rationale}</p>
                  )}
                </div>
                {option.confidence === 'high' && (
                  <span className="text-emerald-600 text-xs font-medium bg-emerald-50 px-2 py-1 rounded-full whitespace-nowrap">High confidence</span>
                )}
              </label>
            ))}
          </div>
        ...
        {/* Secondary Endpoints */}
        <Card variant="default" padding="md">
          <h2 className="font-semibold text-stone-900 mb-1">Secondary Endpoints</h2>
          <p className="text-sm text-stone-500 mb-4">What else do you want to track?</p>
          <div className="space-y-3">
            {endpoints
              ...
              .map((option) => (
                ...
                >
                  <input
                    ...
                    className="w-4 h-4 mt-0.5 text-teal-600 border-stone-300 rounded focus:ring-teal-500"
                  />
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-stone-900 font-medium">{option.name}</span>
        ...
        {/* Duration */}
        <Card variant="default" padding="md">
          <h2 className="font-semibold text-stone-900 mb-1">Study Duration</h2>
          <p className="text-sm text-stone-500 mb-4">How long will the study run?</p>
          <div className="space-y-3">
            {DURATION_OPTIONS.map((option) => (
              ...
              >
                <input
                  ...
                  className="w-4 h-4 text-teal-600 border-stone-300 focus:ring-teal-500"
                />
                <span className="flex-1 text-stone-900 font-medium">{option.label}</span>
                {discoveryData?.recommendedDuration?.weeks === option.weeks && (
                  <span className="text-amber-600 text-xs font-medium bg-amber-50 px-2 py-1 rounded-full">Recommended</span>
                )}
              </label>
            ))}
          </div>
          ...
          {/* Duration info */}
          {durationRationale && (
            <div className="mt-4 flex gap-3 p-4 bg-violet-50 border border-violet-100 rounded-xl text-sm text-violet-700">
        ...
        {discoveryData?.safetyConsiderations && discoveryData.safetyConsiderations.length > 0 && (
          <Card variant="default" padding="md" className="border-amber-200 bg-amber-50">
            <h2 className="font-semibold text-stone-900 mb-3">Safety Considerations</h2>
